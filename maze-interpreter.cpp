...
render_cells(GameState * game_state, Mouse * mouse, FrameBuffer * frame_buffer, Rectangle render_region, V2 screen_offset, QuadTree * tree)
{
  ...
  if (tree && overlaps(render_region, (tree->bounds * game_state->cell_spacing) + screen_offset))
  {
    ...
    render_cells(game_state, mouse, frame_buffer, render_region, screen_offset, tree->top_left);
    render_cells(game_state, mouse, frame_buffer, render_region, screen_offset, tree->bottom_right);
    render_cells(game_state, mouse, frame_buffer, render_region, screen_offset, tree->bottom_left);
  }
}


void
add_initial_cars(QuadTree * tree, Cars * cars)
{
  // IMPORTANT: Too slow!
  ...
  if (tree)
  {
    ...
    add_initial_cars(tree->top_right, cars);
    add_initial_cars(tree->top_left, cars);
    add_initial_cars(tree->bottom_right, cars);
    add_initial_cars(tree->bottom_left, cars);
  }
}
...
render(GameState * game_state, FrameBuffer * frame_buffer, Mouse * mouse)
{
  ...
  render_cells(game_state, mouse, frame_buffer, render_region, screen_offset, &(game_state->maze.tree));
...
init_game(Memory * memory, GameState * game_state, u32 argc, char * argv[])
{
  ...
  add_initial_cars(&(game_state->maze.tree), &(game_state->cars));
}
