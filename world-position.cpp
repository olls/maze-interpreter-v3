...
re_form_world_coord(WorldSpace *result)
{
  ...
  V2 integer_offset = round_down(result->offset);
...
world_coord_to_render_window_coord(RenderWindow *render_window, WorldSpace world_coord)
{
  ...
  V2 recentred_cell_coord = (V2){recentred_cell_x, recentred_cell_y}; // Cells at the extreme distance from the render_origin are cropped here
...
{
  V2 result = world_coord_to_render_window_coord(render_window, (WorldSpace){cell_coord_x, cell_coord_y, (V2){0, 0}});
  return result;
}


WorldSpace
render_window_coord_to_world_coord(RenderWindow *render_window, V2 render_window_coord)
{
  WorldSpace result;
